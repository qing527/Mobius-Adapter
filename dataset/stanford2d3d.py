...
class Stanford2D3D(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        H = inputs["gt_depth"].shape[-2]
        
        top_ratio, bottom_ratio = 0.135, 0.135
        top_h = int(H * top_ratio)
        bottom_h = H - int(H * bottom_ratio)
        top_bottom_mask = torch.ones_like(inputs["val_mask"], dtype=torch.bool)
        top_bottom_mask[:, :top_h, :] = False
        top_bottom_mask[:, bottom_h:, :] = False
        
        final_mask = inputs["val_mask"] & top_bottom_mask
        inputs["val_mask"] = final_mask

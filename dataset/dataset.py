...
def resize_for_input(image, depth, output_shape, intrinsic, canonical_shape, to_canonical_ratio, padding_rgb=[123.675, 116.28, 103.53], mask=None):
    ...
    to_canoical_ratio: ratio to resize the original image to the canonical shape 到标准空间的缩放因子
    """

    # 计算目标输出高/宽与规范形状高/宽的比例
    h, w, _ = image.shape
    resize_ratio_h = output_shape[0] / canonical_shape[0]
    resize_ratio_w = output_shape[1] / canonical_shape[1]
    to_scale_ratio = min(resize_ratio_h, resize_ratio_w) # 取较小的缩放比
    ...
    # 计算resize后需要补齐的padding（上下左右各多少像素）。
    # 采用对称padding，保证内容居中
    ...
    # 如果有mask，同步resize和padding，返回时一并输出
    if mask is not None:

...
from models.MobiusAdapter import MobiusAdapter as ControlNetModel
...
def log_validation(vae,text_encoder,tokenizer,unet,controlnet,args,accelerator,weight_dtype,scheduler,epoch,val_loader,tb_tracker=None, global_step=0
                   ):
    ...
    if controlnet is not None:
        pipeline = PanoControlDepthPipeline.from_pretrained(pretrained_model_name_or_path=args.pretrained_model_name_or_path, 
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:  
        ...
        loaded_params = torch.load(os.path.join(args.controlnet_model_name_or_path, 'stage2.pth'))
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
            ...
            if accelerator.is_main_process:
                ...
                while len(weights) > 0:
                    ...
                    if model.__class__.__name__ == "ControlNetModel":
                        ...
                        torch.save(state_dict, os.path.join(output_dir, sub_dir, 'stage2.pth'))
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                ...
                if model_name == "ControlNetModel":
                    sub_dir = "controlnet"
                    load_path = os.path.join(input_dir, sub_dir)
                    
                    param_path = os.path.join(load_path, 'stage2.pth')

                    if os.path.exists(param_path):
                        saved_params = torch.load(param_path, map_location=accelerator.device)
                        model.load_state_dict(saved_params, strict=True)
                    else:
                        raise FileNotFoundError(f"ControlNet 权重文件不存在: {load_path}")

                if model_name == "UNet2DConditionModel":
                    sub_dir = "unet"
                    load_path = os.path.join(input_dir, sub_dir)
                    if os.path.exists(load_path):
                        
                        load_model = UNet2DConditionModel.from_pretrained(load_path)
                        model.register_to_config(**load_model.config)
                        model.load_state_dict(load_model.state_dict(), strict=True)
                        del load_model
                    else:
                        raise FileNotFoundError(f"UNet 权重文件不存在: {load_path}")
    ...
    if accelerator.is_main_process and initial_global_step == 0:
        # unet.eval()
        controlnet.eval()
        log_validation(
            vae=vae,
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                down_block_res_samples, mid_block_res_sample = controlnet(
                    ...
                    controlnet_cond=rgb.to(dtype=weight_dtype),
                    return_dict=False,
                )
                ...
                # weight loss
                h = noise_pred.shape[-2]
                w = noise_pred.shape[-1]
                ...
                weight_matrix = weight_matrix.to(noise_pred.device)
                loss = torch.mean(
                    weight_matrix * (noise_pred.float() - target.float()) ** 2
                )
    ...
    if accelerator.is_main_process:
        ...
        torch.save(controlnet.state_dict(), os.path.join(args.output_dir,'stage2.pth'))
